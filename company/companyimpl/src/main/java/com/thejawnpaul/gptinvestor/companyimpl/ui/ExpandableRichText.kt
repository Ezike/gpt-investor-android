...
/**
 * An expandable text component that provides access to truncated text with a dynamic ... Show
...
fun ExpandableRichText(
  ...
  Box(
    ...
    RichText(modifier = textModifier.fillMaxWidth().animateContentSize()) {
      val textToShow =
        if (isExpanded) {
          text
        } else {
          text.substring(startIndex = 0, endIndex = text.length.div(6)).dropLastWhile {
