...
class CompanyRepositoryImpl @Inject constructor(
    ...
    override fun getAllCompanies(): Flow<List<Company>> = channelFlow {
        launch {
            companyDao
                .getAllCompaniesFlow()
                .map { entityList ->
        ...
        val cacheModel = companyService.getCompanies().map { model ->
            with(model) {
                CompanyEntity(
                    ticker = ticker,
                    summary = summary,
    ...
    override suspend fun getAllSector(): List<SectorInput> {
        ...
        return if (companyList.isEmpty()) {
            ...
            val sectors = companyList.map { entity ->
                SectorInput.CustomSector(
                    sectorName = entity.sector,
                    sectorKey = entity.sectorKey
                )
            }.filterNot { it.sectorName.lowercase() == "n/a" }.toSet().take(5)
            allSector + sectors
        }
    }
